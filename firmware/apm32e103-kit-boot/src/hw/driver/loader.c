...
uint32_t loaderDownToFlash(uint32_t offset_addr, firm_tag_t *p_tag_arg,  uint32_t timeout_ms)
{
    ...
    if (ymodemReceive(&ymodem) == true)
    {
      ...
      switch(ymodem.type)
      {
        ...
        case YMODEM_TYPE_DATA:
          flash_addr = FLASH_ADDR_FIRM + FLASH_SIZE_TAG + ymodem.file_addr;
      ...
      if (lcdIsInit())
      {
        if (lcdDrawAvailable() || ymodem.type == YMODEM_TYPE_END)
        {
...
void cliCmd(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "info"))
  {
    ...
    firm_ver_t *p_firm = (firm_ver_t *)(FLASH_ADDR_FIRM + FLASH_SIZE_TAG + FLASH_SIZE_VEC);
  ...
  if (args->argc == 1 && args->isStr(0, "flash"))
  {
    ...
    loaderDownToFlash(0, NULL, 30000); 
    ret = true;
  }
