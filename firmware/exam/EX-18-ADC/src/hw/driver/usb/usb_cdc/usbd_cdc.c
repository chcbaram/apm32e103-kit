  ...
  switch (reqType)
  {
    ...
    case USBD_REQ_TYPE_CLASS:
      if (wLength)
      {
        ...
        {
          ((USBD_CDC_INTERFACE_T *)usbInfo->devClassUserData[usbInfo->classID])->ItfCtrl(request, (uint8_t *)usbDevCDC->data, wLength);
        ...
        else
        {
          ...
          usbDevCDC->cdcCmd.length = wLength < USBD_EP0_PACKET_MAX_SIZE ? wLength : USBD_EP0_PACKET_MAX_SIZE;
      ...
      {
        ((USBD_CDC_INTERFACE_T *)usbInfo->devClassUserData[usbInfo->classID])->ItfCtrl(request, (uint8_t *)req, 0);
      }
      break;
...
static USBD_STA_T USBD_CDC_RxEP0Handler(USBD_INFO_T *usbInfo)
{
  ...
  {
    ((USBD_CDC_INTERFACE_T *)usbInfo->devClassUserData[usbInfo->classID])->ItfCtrl(usbDevCDC->cdcCmd.opcode, (uint8_t *)usbDevCDC->data, (uint16_t)usbDevCDC->cdcCmd.length);
...
static USBD_STA_T USBD_CDC_DataInHandler(USBD_INFO_T *usbInfo, uint8_t epNum)
{
...
#if defined(USE_DAL_DRIVER)
  if ((usbInfo->devEpIn[epNum & 0x0F].length > 0) && (usbInfo->devEpIn[epNum & 0x0F].length % usbdh->IN_ep[epNum & 0x0F].maxpacket) == 0)
#else
  if ((usbInfo->devEpIn[epNum & 0x0F].length > 0) && (usbInfo->devEpIn[epNum & 0x0F].length % usbdh->epIN[epNum & 0x0F].mps) == 0)
#endif /* USE_DAL_DRIVER */
  {
  ...
  else
  {
    ...
    {
      ((USBD_CDC_INTERFACE_T *)usbInfo->devClassUserData[usbInfo->classID])->ItfSendEnd(epNum, usbDevCDC->cdcTx.buffer, &usbDevCDC->cdcTx.length);
    }
  }
...
static USBD_STA_T USBD_CDC_DataOutHandler(USBD_INFO_T *usbInfo, uint8_t epNum)
{
  ...
  ((USBD_CDC_INTERFACE_T *)usbInfo->devClassUserData[usbInfo->classID])->ItfReceive(usbDevCDC->cdcRx.buffer, &usbDevCDC->cdcRx.length);
