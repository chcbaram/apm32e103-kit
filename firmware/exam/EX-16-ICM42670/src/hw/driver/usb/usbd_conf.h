...
typedef struct
{
  ...
  void       *classData;
  USBD_STA_T (*ClassInitHandler)
  (struct _USBD_INFO_T *usbInfo, uint8_t cfgIndex);
  USBD_STA_T (*ClassDeInitHandler)
  (struct _USBD_INFO_T *usbInfo, uint8_t cfgIndex);
  USBD_STA_T (*ClassSofHandler)
  (struct _USBD_INFO_T *usbInfo);

  /* Control endpoint */
  USBD_STA_T (*ClassSetup)
  (struct _USBD_INFO_T *usbInfo, USBD_REQ_SETUP_T *req);
  USBD_STA_T (*ClassTxEP0)
  (struct _USBD_INFO_T *usbInfo);
  USBD_STA_T (*ClassRxEP0)
  (struct _USBD_INFO_T *usbInfo);
  /* Specific endpoint */
  USBD_STA_T (*ClassDataIn)
  (struct _USBD_INFO_T *usbInfo, uint8_t epNum);
  USBD_STA_T (*ClassDataOut)
  (struct _USBD_INFO_T *usbInfo, uint8_t epNum);
  USBD_STA_T (*ClassIsoOutIncomplete)
  (struct _USBD_INFO_T *usbInfo, uint8_t epNum);
  USBD_STA_T (*ClassIsoInIncomplete)
  (struct _USBD_INFO_T *usbInfo, uint8_t epNum);
} USBD_CLASS_T;
