...
typedef enum
{
  ...
  /**
   * @brief    SDIO standard error defines
   */
  SD_INTERNAL_ERROR,
  SD_NOT_CONFIGURED,
  SD_REQUEST_PENDING,
...
typedef struct
{
  ...
  uint8_t  MaxWrCurrentVDDMax;  /*!< Max. write current @ V*D max */
...
typedef struct
{
  SD_CSD_T  SD_csd;
  SD_CID_T  SD_cid;
  long long CardCapacity;  /*!< SD Crad Capacity. Maximun support 2^64 bytes */
  uint32_t  CardBlockSize; /*!< SD Card Sector Size */
  uint16_t  RCA;           /*!< Ralative card address */
...
/* SDIO transmission frequency  */
#define SDIO_TRANSFER_CLK_DIV                     0x04

/* SDIO data address  */
#define SDIO_DATA_ADDRESS                         (uint32_t) & SDIO->FIFODATA
